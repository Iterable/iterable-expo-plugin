...
import fs from 'fs';
import {
  addAndroidAppDependency,
  addAndroidProjectDependency,
  addAndroidApplyPlugin,
} from '../utils';
  ...
  (config) => {
    return withProjectBuildGradle(config, async (newConfig) => {
      if (newConfig.modResults.language === 'groovy') {
        newConfig.modResults.contents = addAndroidProjectDependency(
...
const withFirebaseInAppBuildGradle: ConfigPlugin<ConfigPluginPropsWithDefaults> =
  (config) => {
    return withAppBuildGradle(config, (newConfig) => {
      if (newConfig.modResults.language === 'groovy') {
        newConfig.modResults.contents = addAndroidApplyPlugin(
          newConfig.modResults.contents,
          GOOGLE_SERVICES_PLUGIN
        );
        newConfig.modResults.contents = addAndroidAppDependency(
          newConfig.modResults.contents,
          {
