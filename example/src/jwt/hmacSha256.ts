...
function sha256(message: Uint8Array): Uint8Array {
  ...
  const paddedLen = ((message.length + 9 + 63) & ~63) >>> 0;
...
/**
 * HMAC-SHA256 over UTF-8 strings. Pure JS so Hermes / RN can sign JWTs
 * without Web Crypto SubtleCrypto.
 */
export function hmacSha256(secret: string, data: string): Uint8Array {
  ...
  for (let i = 0; i < HMAC_BLOCK_SIZE; i += 1) {
    ipad[i] = (ipad[i] ?? 0) ^ 0x36;
    opad[i] = (opad[i] ?? 0) ^ 0x5c;
  }

  const inner = new Uint8Array(HMAC_BLOCK_SIZE + dataBytes.length);
  inner.set(ipad);
  inner.set(dataBytes, HMAC_BLOCK_SIZE);
